...
{
	printf("msg %s %s\n", Timestampff::now().toFormatString().c_str(), msg);
	if (++cnt == 20)
	{
		g_loop->quit();
	}
...
int main()
{
	...
	sleep(1);
	{
		...
		TimerIdff t45 = loop.runAfter(4.5, std::bind(print, "once4.5"));
		loop.runAfter(4.2, std::bind(cancel, t45));
		loop.runAfter(4.8, std::bind(cancel, t45));
		loop.runEvery(2, std::bind(print, "every2"));
		TimerIdff t3 = loop.runEvery(3, std::bind(print, "every3"));
	...
	//sleep(1);
	/*{
		EventLoopffThread loopThread;
		EventLoopff* loop = loopThread.startLoop();
